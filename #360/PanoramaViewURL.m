...
GLKQuaternion GLKQuaternionFromTwoVectorsURL(GLKVector3 u, GLKVector3 v){
...
@interface PanoramaViewURL (){
	SphereURL *sphere, *meridians;
...
@implementation PanoramaViewURL
...
-(id) initWithFrame:(CGRect)frame context:(EAGLContext *)context{
	...
	if (self) {
		...
		sphere = [[SphereURL alloc] init:48 slices:48 radius:10.0 textureFile:nil];
		meridians = [[SphereURL alloc] init:48 slices:48 radius:8.0 textureFile:@"equirectangular-projection-lines.png"];
	}
	return self;
}
...
-(void) setImageWithData:(NSURL *)URL{ }
...
-(void) panHandler:(UIPanGestureRecognizer*)sender{
	...
	if([sender state] == 1){
		...
		touchVector = [self vectorFromScreenLocation:location inAttitude:_attitudeMatrix];
	}
		...
		GLKVector3 nowVector = [self vectorFromScreenLocation:location inAttitude:_attitudeMatrix];
		GLKQuaternion q = GLKQuaternionFromTwoVectorsURL(touchVector, nowVector);
