...
// ViewController
class Videoviewer: UIViewController, SCNSceneRendererDelegate, UIGestureRecognizerDelegate {
    
    
    @IBOutlet weak var leftSceneView                : SCNView!
    @IBOutlet weak var rightSceneView               : SCNView!
    
    @IBOutlet weak var VR                           : UIImageView!
    ...
    override func viewDidLoad() {
        ...
        recognizer                                  = UITapGestureRecognizer(target: self, action:#selector(Videoviewer.tapTheScreen))
        recognizer!.delegate                        = self
        view.addGestureRecognizer(recognizer!)
        
        panRecognizer                               = UIPanGestureRecognizer(target: self, action: #selector(Videoviewer.panGesture(_:)))
    ...
    func play(){
        ...
        let fileURL: NSURL? = NSURL(string: videoname)
...
//        let fileURL: URL? = URL(fileURLWithPath: Bundle.main.path(forResource: videoName, ofType: "MP4")!)
    ...
    fileprivate func displayIfNeededCardboardView() {
        VR.isHidden = (false == cardboardViewOn)
        ...
        cardboardButton?.setImage(UIImage(named: (true == cardboardViewOn) ? "cardboardOff" : "cardboardOn"), for: UIControlState())
